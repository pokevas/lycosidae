...
namespace hide_string
{
	...
	protected:
		...
		static void xtea3_data_crypt(uint8_t* inout, const uint32_t len, const bool encrypt, const uint32_t* key)
		{
			...
			{
				copy_memory(data_array, inout, block_size);
				if (encrypt)
				{
				...
				copy_memory(inout, data_array, block_size);
		...
		uint8_t* data_crypt(const uint8_t* data, const uint32_t key[8], const uint32_t size)
		{
			...
			copy_memory(data_ptr_, reinterpret_cast<char*>(&size_crypt_), 4);
			copy_memory(data_ptr_ + 4, reinterpret_cast<char*>(&size_decrypt_data_), 4);
		...
		uint8_t* data_decrypt(const uint8_t* data, const uint32_t key[8], const uint32_t size)
		{
			...
			if (size_crypt_ <= size)
			{
				...
				copy_memory(data_ptr_, data + 8, size_crypt_ - 8);
